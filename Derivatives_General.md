...
| Digital (cash-or-nothing) | No (terminal only) | No (typically European) | No (discontinuous at strike, around strike a small change of price can lead to payoff jumps from 0 → 1 and vice versa.) | Closed form (BS); replication via vanillas |
| Barrier knock-out | Yes (barrier hit event) | No (standard barriers European) | No (discontinuous around barrier / event, same as Digital around strike) | PDE-FDM; MC with Brownian bridge (bias correction); FFT in some models |
